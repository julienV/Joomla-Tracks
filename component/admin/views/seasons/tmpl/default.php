...
$saveOrderUrl = 'index.php?option=com_tracks&task=seasons.saveOrderAjax&tmpl=component';
...
<script type="text/javascript">
	...
	submitbutton = function (pressbutton)
	{
		...
		if (pressbutton == 'seasons.delete')
		{
			var r = confirm('<?php echo JText::_("COM_TRACKS_SEASON_DELETE_COMFIRM")?>');
...
<form action="index.php?option=com_tracks&view=seasons" class="admin" id="adminForm" method="post" name="adminForm">
	<?php
				...
				<tr>
					...
					<td>
						<?php if ($this->canEditState) : ?>
							<?php echo JHtml::_('rgrid.published', $row->published, $i, 'seasons.', true, 'cb'); ?>
						<?php else : ?>
					...
					<?php if ($this->canEdit) : ?>
						<td>
							<?php if ($row->checked_out) : ?>
								...
								echo JHtml::_('rgrid.checkedout', $i, $editor->name, $row->checked_out_time, 'seasons.', $canCheckin);
								?>
							<?php endif; ?>
						</td>
					<?php endif; ?>
					...
					<td>
						...
						<?php if (($row->checked_out) || (!$this->canEdit)) : ?>
							<?php echo $itemTitle; ?>
						<?php else : ?>
							<?php echo JHtml::_('link', 'index.php?option=com_tracks&task=season.edit&id=' . $row->id, $itemTitle); ?>
						<?php endif; ?>
					</td>
					<td>
						<?php echo $row->id; ?>
