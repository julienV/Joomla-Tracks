...
// 		$country["SLE"]= Array( "iso2" => "SL", "name" => "Sierra Leone, Republic of");
		...
		$country["SVK"]= Array( "iso2" => "SK", "name" => "Slovakia");
...
// 		$country["UGA"]= Array( "iso2" => "UG", "name" => "Uganda, Republic of");
		...
		$country["GBR"]= Array( "iso2" => "GB", "name" => "United Kingdom");
...
// 		$country["ZWE"]= Array( "iso2" => "ZW", "name" => "Zimbabwe, Republic of");
  ...
  function convertIso2to3($iso_code_2)
	 ...
	 * @param $iso_code
	 */
	function getIsoFlag($iso_code,$size)
	{
		if (strlen($iso_code) == 3) {
			$iso_code = self::convertIso3to2($iso_code);
		}
		if ($iso_code) {
			$path = JURI::base().'media/com_tracks/images/flags'.($size?'-'.$size:'').'/'.strtolower($iso_code).($size=='m'?'.png':'.gif');
			return $path;
		}
		else return null;
	}
 ...
 /**
	 ...
	 */
	function getCountryFlag($countrycode, $size=null , $attributes = null, $link = true)
	{
		$src = self::getIsoFlag($countrycode,$size);
		if (!$src) {
			return '';
		}
		$attr = array('title' => 'see riders from ' . self::getCountryName($countrycode));
		if ($attributes) {
			$attr = array_merge($attr, $attributes);
		}
		$img = JHTML::image($src, self::getCountryName($countrycode), $attr);
		if (!$link) {
			return $img;
		}
		else {
			$link = TracksHelperRoute::getIndividualsRoute('search').'&country='.$countrycode;
			return JHTML::link($link, $img);
		}
	}
