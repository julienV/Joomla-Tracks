...
<form action="index.php?option=com_tracks&amp;controller=individual&amp;task=element&amp;tmpl=component" method="post" name="adminForm" id="adminForm">
<table>
<tr>
...
<div id="editcell">
	...
	for ($i=0, $n=count( $this->items ); $i < $n; $i++)
	{
		...
		<tr class="<?php echo "row$k"; ?>">
			<td>
				<a class="pointer" onclick="if (window.parent) window.parent.<?php echo $this->escape($function);?>('<?php echo $row->id; ?>', '<?php echo $this->escape(addslashes($row->last_name.', '.$row->first_name)); ?>');"><?php echo $this->escape($row->last_name); ?></a>
			</td>
			<td>
				<?php
				echo $row->first_name;
